    ...
    return (
        <>
            ...
            <div className="w-full">
                ...
                <div className="flex min-h-[calc(100vh-40px)] flex-col items-start justify-start md:justify-center bg-[var(--background)] p-8 md:flex-row gap-8">
                    <div className="w-full rounded bg-white p-8 shadow md:m-0 md:w-3/10">
                        <form className="space-y-6" onSubmit={handleSubmit}>
                            <div className="space-y-2">
                                ...
                                <RadioGroup value={formType} onValueChange={handleTypeChange} className="flex flex-wrap gap-4">
                            ...
                            <div className="space-y-2">
                                <Input
                                    id="search"
                                    placeholder={formType === 'people' ? 'e.g. Chewbacca, Yoda, Boba Fett' : 'e.g. A New Hope, Empire Strikes Back'}
                                    value={formInput}
                    ...
                    <div className="flex min-h-96 w-full flex-col rounded bg-white p-8 shadow md:m-0 md:w-4/10">
                        <h2 className="text-xl font-bold">Results</h2>
                        <hr className="my-2" />
                        {!searchTerm ? (
                            <CenteredText>Use the form to search for People or Movies.</CenteredText>
                        ) : searchType === 'people' ? (
                            <ResultsList isLoading={isPeopleLoading} results={peopleData} propertyKey="name" type="people" />
                        ) : (
                            <ResultsList isLoading={isMoviesLoading} results={moviesData} propertyKey="title" type="movies" />
                        )}
                    </div>
                </div>
            </div>
